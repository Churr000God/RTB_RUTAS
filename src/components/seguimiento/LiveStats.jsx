...
const Stat = ({ label, value }) => (
  <div className="rounded-lg border border-rtb-teal-100 bg-white px-2 py-2">
    ...
    <div className="font-mono text-sm text-rtb-navy">{value}</div>
  </div>
);
...
export default function LiveStats({ state, allPoints, segments, waits }) {
  ...
  return (
    <div>
      <div className="mb-2">
        <div className="mb-1 flex items-center justify-between text-[10px] uppercase tracking-wider text-rtb-navy-mid">
          <span>Progreso</span>
          <span className="font-mono normal-case text-rtb-navy-mid">{route.length}/{total || route.length}</span>
        </div>
